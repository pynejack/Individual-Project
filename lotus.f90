...
! -- Define parameter and declare variables
  ...
  ! physical parameters
  real,parameter     :: U = 1, Re = 7000, nu=U*c/Re, St = 0.4
  real,parameter     :: hamp = c, freq = St*U/(2*hamp), pamp = atan(pi*St)-pi/18.
  real,parameter     :: hamp2 = c, freq2 = St*U/(2*hamp), pamp2 = atan(pi*(2*hamp2*freq2/U))-pi/18
  real,parameter     :: spacing = 3*c, phase = pi
  ! variables
  real :: force1(3),force2(3),moment1(3),moment2(3)
...
! -- Initialize domain size and placement
  ...
  call xg(1)%stretch(n(1)*b(1), -5*c, -0.4*c,   6*c, 12*c, h_max=10., prnt=root) ! x-grid
  call xg(2)%stretch(n(2)*b(2), -5*c, -2.1*c, 2.1*c,  5*c,            prnt=root) ! y-grid
!
...
!
! -- Time update loop
  do while (flow%time*freq<10.and..not.there)  ! run 10 cycles or until kill switch
